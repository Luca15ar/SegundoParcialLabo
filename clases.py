...
class Nave_Enemiga:
    def __init__(self, x, y, ruta, escala, velocidad = None, sonido = None) -> None:
        self.ruta= ruta
        self.lista_animacion = []
        self.frame_index = 0
        self.accion = 0 #0 -> Inactivo 1-> Muerto
        ...
        for i in range(12):
            img = pygame.image.load(f"Segundo_Parcial/Img_juego/{self.ruta}/Idle/{i}.png")
            img = pygame.transform.scale(img, (img.get_width() * escala, img.get_height() * escala))
            lista_temp.append(img)
        ...
        for i in range(12):
            img = pygame.image.load(f"Segundo_Parcial/Img_juego/{self.ruta}/Explosion-sprite/{i}.png")
            img = pygame.transform.scale(img, (img.get_width() * 1, img.get_height() * 1))
            lista_temp.append(img)
    ...
    def movimiento(self):
        #============================Verifico si la nave enemiga toca el borde tantas veces=============================
        if(self.contador < 2):
    ...
    def ataque(self):
        if(randint(0, 250) < self.rango_disparo):
            self.disparo()

    def disparo(self):
        x, y = self.rect.center
        proyectil = Proyectil(x, y, "Segundo_Parcial/Img_juego/Enemy-ship-1/Shoot-sprite-enemy", False, 5, 0.5)
        self.lista_disparos.append(proyectil)
